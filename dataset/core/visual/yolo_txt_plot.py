...
class Visualizer:
    ...
    def draw_pred_boxes(self, img, lines):
        boxes = []
        for line in lines:
            line = line.strip().split(" ")
            if len(line) == 6:
    ...
    def draw_gt_boxes(self, img, lines):
        boxes = []
        for line in lines:
            line = line.strip().split(" ")
            if len(line) == 5:
    ...
    def draw_boxes(self, img_path, pred_path):
        ...
        pred_boxes = self.draw_pred_boxes(img, lines)
        gt_boxes = self.draw_gt_boxes(img, lines)
    ...
    def draw_boxes_folder(self, img_folder, pred_folder, output_folder, pic_nums=None):
        ...
        img_list = [file for file in img_list if file.endswith(('.jpg', '.png'))]
